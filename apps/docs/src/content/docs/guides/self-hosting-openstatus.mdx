...
7.  **Create a Workspace**
    ...
    -   When `SELF_HOST=true` is set, every workspace automatically runs on the "Pro" plan with all paid features unlocked and the following limits applied: 100 monitors, 20 status pages, 50 notification channels, unlimited members, 24-month data retention, all periodicities (down to 30s), all regions, private locations, audit log, white-label, SMS, PagerDuty, and more. No manual database update is required.

    If you want to tune any limit for a specific workspace, you can still override it per-workspace by updating the `limits` JSON column on the `workspace` row — those values take precedence over the self-host defaults. For most self-hosted users, the defaults are sufficient.
